...
This produces `dist/TeXLib-Installer-v0.1.0.zip` and `dist/SHA256SUMS`. Upload both to a new GitHub Release.
...
- Pre-flight checks live in section 3 of `install.ps1`. Add new checks via `Add-PreflightFailure` (blocks install) or `Add-PreflightWarning` (advisory only).
