...
One-click portable Windows installer for the [TeXLib](https://github.com/landonfox00/TeXLib) teaching library. It sets up Sublime Text, SumatraPDF, and TeX Live under `%LOCALAPPDATA%\TeXLib` (no admin rights needed), wires up LaTeXTools with the TeXLib custom builder, and deploys the library so `\documentclass{didactic}`, `{quiz}`, `{autoexam}`, and the rest work out of the box.

> Looking to **install** TeXLib on a coworker's machine? See the [install guide](INSTALL.md). This README is for people maintaining the installer itself.
...
Everything lands under one root, which is what makes uninstall a single directory removal. Before 0.6.3 the library went to `<OneDrive>\Documents\TeXLib`; see the [changelog](CHANGELOG.md) for why it moved and how upgrades migrate.
...
`tools/uninstall-gui.ps1` is the same arrangement over `uninstall.ps1`. Per-component removal is not new — the console has confirmed Sublime Text, SumatraPDF, TeX Live, and the library separately since 0.6.x — the window puts a form over the `-Keep*` switches that choice already had. Note the polarity: the checkboxes say **remove**, the switches say **keep**, so the GUI inverts them in one place and CI asserts the wiring, because getting it backwards deletes a 6 GB tree someone meant to keep.

The coupling that can rot silently is each GUI's phase table: they recognize the console banners to drive their progress bars, so renaming a banner leaves a run that works correctly behind a bar that never moves. The `gui` CI job asserts every marker still tracks its script — literally for the ones printed as constants, and via a declared `Emit` construct for those composed at runtime (`"Downloading $($Info.File)..."`, `"Removing $Label ($Path)..."`).
...
| `-OnlyTeXLib` | Fetch the pinned TeXLib library and refresh the Sublime builder files. Skips Sublime, Sumatra, and TeX Live entirely. |
...
Combine flags as needed (for example, `-OnlyTeXLib -Silent` for unattended library refreshes on lab machines).
...
| `-KeepSublime` / `-KeepSumatra` / `-KeepTeXLive` | Leave that component in place. Keeping TeX Live saves re-downloading about 6 GB, which is typically 30-60 minutes. |
...
An interactive run asks about each component, so you need the flags only for unattended use.
...
- The installer **must** run cleanly with `-Silent` (no `Read-Host` prompts), because lab-machine deployment relies on silent mode.
...
- `PSScriptAnalyzer` runs on every push via `.github/workflows/lint.yml`. Run it locally before pushing:
  ```powershell
...
Different audiences. TeXLib's users are developers of the library (forking, editing `.sty` files, contributing back). Installer users are *consumers* of the resulting setup — they don't need Git, the smoke test, or the changelog. Keeping them separate keeps each surface clean.
